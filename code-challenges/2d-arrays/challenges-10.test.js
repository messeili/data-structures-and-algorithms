...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => {
    res.status(200).send('This is Mohammad Yahyia');
  });

  app.get('/favoritefoods', (req, res) => {
    let favoriteFoodsArr = ['Mansaf', 'Burger', 'shawerma'];
    res.status(200).send(favoriteFoodsArr);
  });

  app.get('*', (req, res) => {
    res.status(404).send('There is an error with your route');
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((arr) => {
    return arr.filter(element => {
      if (!(isNaN(element) || (element % 5 !== 0)) && (typeof (element) !== 'string')) {
        return element;
      }
    });
  ...
  // return input.map(val => val.map(num => Math.pow(2, num)))
  // Solution code here...
};
/* ------------------------------------------------------------------------------------------------
